function(task, responses){
    if(task.status.includes("error")){
        ...
        const normalizeWindowsPath = function(path) {
            return String(path || "")
                .replace(/\//g, "\\")
                .replace(/\\+$/, "")
                .toLowerCase();
        ...
        const standardRootDirectoryStyle = {
        ...
        const highlightedRootDirectories = new Set([
            "c:\\windows",
            "c:\\users",
            "c:\\program files",
            "c:\\program files (x86)",
        ...
        let createFormattedRow = function(data, entry, isCRootListing = false) {
            let entryStyling = lookupEntryStyling(entry);
            const isHighlightedRootDirectory = isCRootListing &&
                !entry["is_file"] &&
                highlightedRootDirectories.has(normalizeWindowsPath(entry["full_name"]));
            return {
                rowStyle: isHighlightedRootDirectory ? standardRootDirectoryStyle : {},
                name: {
        ...
        for (let i = 0; i < responses.length; i++) {
            ...
            if (data["is_file"]) {
                ...
                formattedResponse.rows.push(createFormattedRow(data, data, false));
            } else {
                const isCRootListing = normalizeWindowsPath(ls_path) === "c:";
                formattedResponse.rows = formattedResponse.rows.concat(
                    data["files"]?.map((entry) => createFormattedRow(data, entry, isCRootListing))
                );
            }
        }
